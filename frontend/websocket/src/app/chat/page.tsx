...
const Chat: React.FC = () => {
  ...
  const [msgs, setMsgs] = useState<
    { text: string; sender: string; receiver: string; right: boolean }[]
  >([]);
  ...
  const sendMsg = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (socket) {
      ...
      setMsgs((prevMsgs) => [...prevMsgs, { ...msgToBeSent, right: true }]);
      setMsg("");
    }
  };
      ...
      <div className="space-y-2">
        ...
        <div
          ...
          className={`p-3 cursor-pointer hover:bg-green-500 dark:hover:bg-green-600 border-b border-green-500 ${
            receiver === user?.username ? "bg-green-500 dark:bg-green-600" : ""
          }`}
        >
          {user?.username}
        </div>
